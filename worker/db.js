...
CREATE TABLE IF NOT EXISTS vault_secrets(id TEXT PRIMARY KEY,label TEXT NOT NULL,label_norm TEXT NOT NULL UNIQUE,username TEXT,secret_ct TEXT NOT NULL,iv TEXT NOT NULL,created_by TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,last_accessed_at TEXT);
CREATE INDEX IF NOT EXISTS idx_vault_label ON vault_secrets(label_norm);
CREATE TABLE IF NOT EXISTS vault_events(id TEXT PRIMARY KEY,secret_id TEXT,person_id TEXT NOT NULL,action TEXT NOT NULL,created_at TEXT NOT NULL);
